...
<div class="layui-card">
    <div class="layui-card-header header-title">应用场景列表</div>
    <div class="layui-card-body">
        <table class="layui-hide" id="test-table-toolbar" lay-filter="test-table-toolbar"></table>

        <script type="text/html" id="test-table-toolbar-toolbarDemo">
            <div class="layui-btn-container">
                <a href="/photo-type/info" class="layui-btn layui-btn-sm">添加应用场景</a>
            </div>
        </script>
...
<script>
    layui.config({
        ...
        table.render({
            ...
            ,url: '/photo-type/index'
            ,cols: [[
                {field:'id', width:80, title: 'ID', sort: true}
                ,{field:'name',width:100, title: '类型名称'}
                ,{field:'imgsrc', title: '列表图',templet: function (d) {
                        return '<a class="theme js_banner_url" target="_blank" href='+d.imgsrc+' >'+d.imgsrc+'</a>';
                  }}
        ...
        table.on('tool(test-table-toolbar)', function(obj){
            ...
            if(obj.event === 'del'){
                layer.confirm('您确定要删除吗', function(index){
                    obj.del();
                    layer.close(index);
                    $.post('/photo-type/delete',{id:data.id},function(res){
                        if(res.code == 100000){
                ...
                window.location.href = '/photo-type/info/?id='+data.id;
            }
        });
    });
</script>