    ...
    <div class="personal-right">
        ...
        <div class="fy-sub-nav">
            <?php foreach ($data['typeArr'] as $key => $type) { ?>
                <a class="sub-nav-item <?=$type_id == $key ? 'active' : ''?>" href="/user/down-history?type=<?=$key?>"><?=$type['title']?>(<?=$type['number']?>)</a>
            <?php } ?>
        ...
        <div class="user-unit-list">
            <?php if(!empty($data['widget'])){ foreach ($data['widget'] as $val) { ?>
                <div class="user-unit-item ">
                    <a href="<?=$val['download']?>" class="user-unit-edit transition">下 载</a>
